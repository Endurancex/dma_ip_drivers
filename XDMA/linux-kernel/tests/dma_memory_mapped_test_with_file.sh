...
if [ $h2cChannels -gt 0 ]; then
	...
	for ((i=0; i<=3; i++)); do
		...
		$tool_path/dma_to_device -d /dev/${xid}_h2c_${curChannel} \
		       	-f data/datafile${i}_32M.bin -s $transferSz \
...
if [ $c2hChannels -gt 0 ]; then
	...
	for ((i=0; i<=3; i++)); do
		...
		$tool_path/dma_from_device -d /dev/${xid}_c2h_${curChannel} \
		       	-f data/output_datafile${i}_32M.bin -s $transferSz \
		       	-a $addrOffset -c $transferCount &
		#$tool_path/dma_from_device -d /dev/${xid}_c2h_${curChannel} \
		#       	-s $transferSz \
		#       	-a $addrOffset -c $transferCount &
...
#	for ((i=0; i<=3; i++)); do
#		cmp data/output_datafile${i}_32M.bin data/datafile${i}_32M.bin \
#			-n $transferSz
#		returnVal=$?
#	       	if [ ! $returnVal == 0 ]; then
...
#			echo -e "\twrite data file: data/datafile${i}_32M.bin"
#			echo -e "\tread data file:  data/output_datafile${i}_32M.bin"
#			testError=1
#		else
